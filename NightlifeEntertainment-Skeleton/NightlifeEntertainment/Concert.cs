namespace NightlifeEntertainment
{
    ...
    public class Concert : Performance
    {
        ...
        {
            if (!this.Venue.AllowedTypes.Contains(PerformanceType.Opera)
                || !this.Venue.AllowedTypes.Contains(PerformanceType.Concert))
            {
                throw new InvalidOperationException(
                    string.Format(
                        "The venue {0} does not support the type of performance {1}", 
                        this.Venue.Name, 
                        this.Type));
            }
        }
